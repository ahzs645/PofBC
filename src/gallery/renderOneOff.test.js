...
import { ONE_OFF_ROLES, renderOneOffSvg, rolesOf } from './renderOneOff.js'
...
test('every mark has a sun, mountains and the wordmark, lifted from its artwork', () => {
  for (const [id, mark] of Object.entries(ONE_OFF_MARKS)) {
    ...
    for (const role of ['sun', 'rays', 'mountains', 'wordmark']) assert.ok(roles.includes(role), `${id} has no ${role}`)
